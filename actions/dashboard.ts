...
// Define the shape of SQL aggregation results
type MeetingTrendRow = {
    year: number;
    month: string;
    total_meetings: number;
...
// Define type for role aggregation results
type UserRoleGroup = {
    role: string;
    _count: {
        _all: number;
...
export const getDashboardStats = createCached(
    'dashboard-stats',
    async () => {
        // --- Phase 1: simple counts (reduces parallel load)
        const totalUsers = await db.user.count();
        const totalMeetings = await db.meeting.count();
        const totalWhiskies = await db.whisky.count();
        const totalReviews = await db.review.count();

        // --- Phase 2: batch remaining queries (pooled safely)
        const [
            adminCount,
            verifiedUsers,
            openMeetings,
            ...
            db.user.groupBy({
                by: ['role'],
                _count: { _all: true }
            }) as unknown as UserRoleGroup[], // ✅ Explicit cast to fix TypeScript inference
      ...
      `
        ...
        // --- User role distribution (now type-safe)
        const userRoleDistribution = usersByRoleRaw.map((g) => ({
            role: g.role === 'ADMIN' ? 'Admins' : 'Users',
            count: g._count._all,
            percentage: Math.round((g._count._all / totalUsers) * 100)
        }));
