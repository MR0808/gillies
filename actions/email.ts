...
export const sendEmailChangeOTP = async (
    ...
    try {
        ...
        await db.emailChangeRecord.deleteMany({
            where: {
                expiresAt: {
                    lte: new Date()
                }
        ...
        await db.emailChangeRecord.create({
            data: {
                userId: user.id,
                email: currentEmail,
                newEmail,
...
export async function verifyEmailChangeOTP(
    ...
    try {
        ...
        const emailChangeRecord = await db.emailChangeRecord.findFirst({
            where: {
                userId: user.id,
                newEmail,
                expiresAt: {
        ...
        if (emailChangeRecord.otp !== otp) {
            // Increment attempts
            await db.emailChangeRecord.update({
